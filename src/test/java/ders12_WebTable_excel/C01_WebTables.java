...
public class C01_WebTables extends TestBase {
    ...
    public void test01(){
        ...
        List<WebElement> özellik = satirElementleriListesi.get(2).findElements(By.xpath("//div[@class='rt-td']"));
        if (özellik.get(0).getText().equals("Cierra")){
            System.out.println(özellik.get(3).getText());//cierra@example.com
        }
        ...
        System.out.println(getdata(4, 2).getText());//vega getiriyor dogru degil ?
    }

    private  WebElement getdata(int satir,int sutun){
        List<WebElement> satirElementleriListesi = driver.findElements(By.xpath("//div[@class='rt-tr-group']"));

      List<WebElement>   istenenlist=satirElementleriListesi.get(satir-1).findElements(By.xpath("//div[@class='rt-td']"));
       WebElement istenen= istenenlist.get(sutun-1);
